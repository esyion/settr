...
export interface SkillDistributeDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  organizationId: string;
  /** 待分发的 skill 名称(对话框标题展示用)。 */
...
/**
 * org skill 分发对话框编排:打开时拉取组织团队与活跃成员列表注入
 * 通用 DistributeDialog,提交时转发载荷。目标列表失败时降级为
 * 仅可分发到整个组织,并给出提示。
 */
export function SkillDistributeDialog(props: SkillDistributeDialogProps) {
  ...
  return (
    <DistributeDialog
      open={props.open}
      title={props.skillName ? `分发 skill:${props.skillName}` : "分发 skill"}
      busy={props.busy}
      warning={
        targetsError ? "目标列表加载失败,暂只能分发到整个组织" : undefined
      }
      teams={teams}
      members={members}
