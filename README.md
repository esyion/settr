...
- [ ] 增加本地规则重复、冲突、过期引用和无效路径检查。
- [ ] 增加备份列表、备份清理和备份恢复能力。
- [ ] 增加 CLI 的登录、状态、同步、差异、历史和恢复命令。
- [ ] 支持 `CLAUDE.md`、Cursor Rules 和 Copilot Instructions 等规则格式。
- [ ] 支持个人、工作区和项目级规则 profile。
- [ ] 支持规则模块、继承、覆盖、禁用和优先级。
- [ ] 增加 Agent 配置适配器并生成目标工具配置。
- [ ] 增加 GitHub/GitLab 导入导出和 Pull Request 工作流。
- [ ] 增加 VS Code 和 JetBrains 编辑器插件。
- [ ] 增加 AI 规则审查、冲突解释和合并建议。
- [ ] 增加规则模拟器和变更影响分析。
- [ ] 增加组织、团队、项目、成员和 RBAC 模型。
- [ ] 增加规则草稿、审批、发布、回滚和紧急撤回流程。
- [ ] 增加规则签名、可信设备和篡改检测。
- [ ] 增加 SSO、SCIM、企业审计和合规导出。
- [ ] 增加端到端加密、设备密钥圈和恢复密钥流程。
- [ ] 增加 Docker、Compose、Helm、备份恢复和私有化部署。
- [ ] 增加监控、告警、限流、状态页和多区域灾备。
- [ ] 完成 Tauri 安装包签名、自动更新和回滚方案。
